...
@Component({
  selector: 'page-gatepass',
  template: `<ion-content no-padding><ion-list><ion-card (click)="presentModal(d)" no-padding *ngFor="let d of data">
            <ion-item-divider>
          <div item-left no-padding>
            <h4><span class="listtitle">Unit :</span>{{d.unit}}</h4></div>
          <div item-right no-padding>
            <h4><span class="listtitle">Date :</span>{{d.date}}</h4></div>
        </ion-item-divider>
        <ion-item>
          <div item-left no-padding>
